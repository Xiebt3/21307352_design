...
软件设计
    ...
    3.继承：python有继承。由多个类共同实现积累、各种xxxNode子类，持有node（Node），实现基类方法重用
"""
...
class Node:
    ...
    def _get_label(self, icon: str) -> str:
        raise NotImplementedError("Json是抽象类，不应该被直接使用，参考：TreeStyleJsonNode icon:{}".format(icon))

    # @设计模式Template模式，部分方法
    # @继承，抽象方法，xxxJsonNode子类必须覆盖实现
    @abstractmethod
    def _get_child_prefix(self, prefix: str) -> str:
        raise NotImplementedError("Json是抽象类，不应该被直接使用，参考：TreeStyleJsonNode prefix:{}".format(prefix))
